function ip = bfGetInterlivIndex(K)
% функци€ перемешивани€ определенна€ в п.5.3.1.1 TS 38.212
% ip вектор новых значений индексов 

% ipILMax “аблица 5.3.1.1-1 TS 38.212
ipILMax = [0 2 4 7 9 14 19 20 24 25 26 28 31 34 42 45 49 50 51 53 54 56 58 59 61 62 65 66 ...
...
kMaxIL = length(ipILMax); % kMaxIL = 164

if K > length(ipILMax)
    error('5Gnr:get_nr_interliving:UnsupportedBlockLength K should be no greater than 164.');
end
...
k = 1; 
for m = 1 : kMaxIL
    if ipILMax(m) >= kMaxIL - K
        ip(k) = ipILMax(m)-(kMaxIL - K) + 1; % +1 так как в MatLab нумераци€ начинаетс€ с 1
        k = k + 1;
    end
end
