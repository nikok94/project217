...
%bfPolarEncode полярный кодер - базовая функция 5GNR 
  % out = bfPolarEncode(in,E) возвращает полярно кодированные значения
  % входных данных in
  % in вектор столбец длиною K, включая биты CRC, если таковые используются
  % out вектор столбец длины N 
  % varargin - входной параметр переменной длины, включает в себя
  % дополниетльные параметры nMax и iIL
  % nMax - максимальное значение n, задает длину выходных значений N = 2^n
  % определено в п. 7.3.3 TS 38.212
  % iIL - чередование ввода, принимает значения true или false
  % определено в п. 5.3.1.1 TS 38.212
  % E - длина выходной последовательности для согласования скорости
  % п.5.4.1. TS 38.212
  
  narginchk(2,4); % задает количество входных переменных либо 2, либо 4
  %% проверка взходных значений
    if nargin==2
        % работаем низходящей линией (Downlink) 
        % bfPolarEncode(in,E)
  ...
  K = length(in(:,1));
  %% перестановка входных значений п.5.3.1.1 TS 38.212
  if iIL
    IPIndex = bfGetInterlivIndex(K); % индексы чередования 
    inInterleaving = in(IPIndex);
  else
    inInterleaving = in;
  end
  %% находим значение N п.5.3.1 TS 38.212
  ...
  %% UCI для PC-Polar, п 6.3.1.3.1 TS 38.212
    if (K>=18 && K<=25) % п. 5.3.1 TS 38.212
        nPC = 3;
        if ((E - K + 3) > 192)
            nPCwm = 1;
        else
  ...
  %% 
    qTable = polarQTab; % п. 5.3.1.2-1 TS 38.212
    idx = (qTable < N);
 ...
 % Находим Subblock interleaving pattern для Polar rate-matching п. 5.4.1.1.TS 38.212
  jn = zeros(N,1);
...
function checkValidIn(in,E,nMax,iIL)
  ...
  if (K>=18 && K<=25) % если используется PC-Polar
    nPC = 3;
  else
    nPC = 0;
  end
